...
def index():
    if request.method == "POST":
        ...
        if re.match(file_regex, filename) and len(filename) > 0 and len(filename) < 50:
            ...
            res = cur.execute("SELECT * FROM editor WHERE email=:email AND filename=:filename", {
                              "email": email, "filename": filename})

            if not res.fetchall():
                cur.execute("INSERT INTO editor (email, filename) VALUES (:email, :filename)", {
                            "email": email, "filename": filename})
                conn.commit()
                conn.close()
    ...
    if session.get("username"):
        ...
        cur.execute("PRAGMA key='{}'".format(config.db_pwd))
        result = cur.execute(
            "SELECT * FROM editor WHERE email=:email", {"email": email})
